...
import CardTravelUnitComponent from "./CardTravelUnitComponent";
...
const Travels = ({}) => {
    ...
    useEffect(() => {
        if(vehicles && devices){
            ...
            const temp =(vehicles.map(vehicles=>{
                ...
                data.push({prop:"TRetrazado", fileName:'Tiempo Retrazado',value:'2 Hrs.'})
                data.push({prop:"Velocidad", fileName:'Velocidad',value:'55 km/hr'})
                data.push({prop:"Ultima posición", fileName:'Último reporte',value:'2 Hrs.'})
                return{
    ...
    }, [classifications, vehicles,devices])
            ...
            if(currentVehicle['gpsDevices'].length > 0){
                if(currentVehicle['gpsDevices'].length > 1){
                    const temp = ()=>(
                        <>
                            {
                                currentVehicle['gpsDevices'].map(dev => (
                                    <Chip key={`Travels_Chip_${index}`} label={dev} color="success" variant="outlined" className="muiChip-device" />
                                ))
                            }
                        </>
                    );
...
const Cards = ({data}) =>{
    ...
    return(
        <Grid container spacing={0} justifyContent="space-around" style={{alignItems: "left", padding: '9px 0 0 0px'}}>
            {
                data.map((dat,index) => (
                    <Grid key={`Travels_Cards_Grid_${index}`} item xs={6} sm={4} md={3} lg={2} xl={2}
                        style={
                            screenWidth >= 1280  ?(
                    ...
                    >
                        <CardTravelUnitComponent vehicle={dat} data={dat.data.data} name='FV7608' classHeader='unitStatusBlue' />
                    </Grid>
                ))
            }
        </Grid>
